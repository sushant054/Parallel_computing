...
// define the domain and number of partitions
double a = 0, b = 1;
int n = 100; 
// Ensure n is a multiple of 4
if (n % 4 != 0) {
if (rank == 0) {
printf("n should be a multiple of 4 for Boole's rule.\n");
}
MPI_Finalize();
return -1;
}
...
// Reduce all local integrals to obtain the final result
double global_result = 0.0;
MPI_Reduce(&local_result, &global_result, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
if (rank == 0) {
printf("Final result using Boole's Rule: %lf\n", global_result);
}
MPI_Finalize();
return 0;
}
